...
def clean_metabolites(
    ...
    return table.assign(**{
        col: table[col].apply(_clean_smiles_list) for col in cols
    })
...
def connect_metabolites(
    ...
    table = table.assign(**{
        f"connection_{index_labels[i]}": partial(
            _have_shared_chemical_simple, 
            col=f"max_similarity_{index_labels[i]}",
        ) 
        for i, (a, b) in enumerate(product(cols, cols2))
    })
...
def main() -> None:
    ...
    table = (
        connect_metabolites(table, cols=("reactants", "products"), id_col=["_id"])
        .merge(
            id_table, 
            how='left',
        )
        .merge(
            id_table.rename(columns={col: f"{col}_2" for col in id_table}), 
            how='left',
        )
    )
